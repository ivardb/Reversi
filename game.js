...
game.prototype.isValidTransition = function (from, to) {
    console.assert(typeof from === 'string', '%s: Expecting a string, got a %s', 'game.prototype.isValidTransition', typeof from)
    console.assert(typeof to === 'string', '%s: Expecting a string, got a %s', 'game.prototype.isValidTransition', typeof to)
    console.assert(from in game.prototype.states === true, '%s: Expecting %s to be a valid transition state', 'game.prototype.isValidTransition', from)
    console.assert(to in game.prototype.states === true, '%s: Expecting %s to be a valid transition state', 'game.prototype.isValidTransition', to)
...
game.prototype.setStatus = function (w) {
    console.assert(typeof w === 'string', '%s: Expecting a string, got a %s', 'game.prototype.setStatus', typeof w)

    if (game.prototype.isValidState(w) && game.prototype.isValidTransition(this.generalState, w)) {
        this.generalState = w
        console.log('[STATUS: ' + this.id + '] %s', this.generalState)
    } else {
...
game.prototype.addPlayer = function (p) {
    console.assert(p instanceof Object, '%s: Expecting an object (WebSocket), got a %s', 'game.prototype.addPlayer', typeof p)
...
game.prototype.capture = function (board, color, x, y) {
    let capArray = game.prototype.checkMove(board, color, x, y)
    for (var i = 0; i < 8; i++) {
        if (capArray[i] === true) {
